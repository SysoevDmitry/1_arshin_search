# 🔍 ФГИС АРШИН — Приложение для работы с реестром поверок

**Версия:** 2.13 (актуализированная)
**Дата:** 3 марта 2026 г.
**Платформы:** Linux, Windows (GUI + Console)

---

## 📋 ОГЛАВЛЕНИЕ

1. [Описание](#описание)
2. [Быстрый старт](#быстрый-старт)
3. [Структура проекта](#структура-проекта)
4. [Возможности](#возможности)
5. [Документация](#документация)
6. [Версии приложения](#версии-приложения)
7. [Помощь](#помощь)

---

## 📖 ОПИСАНИЕ

Приложение для поиска, сбора и экспорта данных из федеральной государственной информационной системы "Аршин" (ФГИС АРШИН).

**Назначение:**
- 🔍 Поиск по реестру поверок средств измерений
- 📦 Пакетная обработка запросов из Excel
- 📊 Экспорт результатов с полной служебной информацией
- 🚀 Консольная версия для удалённых серверов

---

## 🚀 БЫСТРЫЙ СТАРТ

### 1. Установка зависимостей

```bash
cd /home/dmitry/Документы/АРШИН/Интерфейс/1_arshin_search

# Активация виртуального окружения
source venv/bin/activate

# Установка зависимостей
pip install -r requirements.txt
```

### 2. Запуск GUI версии

```bash
python arshin_app.py
```

### 3. Запуск консольной версии

```bash
# Пакетный поиск
python arshin_app_konsol.py -f тесты/Тестовый_запрос_2.xlsx -y 2023,2024,2025

# С экспортом
python arshin_app_konsol.py -f запросы.xlsx -y 2020-2025 -o результат.csv

# Помощь
python arshin_app_konsol.py --help
```

---

## 📁 СТРУКТУРА ПРОЕКТА

```
1_arshin_search/
├── 📄 arshin_app.py              # GUI приложение (Tkinter)
├── 📄 requirements.txt           # Зависимости Python
├── 📄 README.md                  # Этот файл
├── 📄 HELP.md                    # Помощь пользователя
├── 📄 ПРЕЗЕНТАЦИЯ.md             # Презентация проекта
│
├── 📂 Doc/                       # Документация
│   ├── README_KONSOL.md          # Руководство по консольной версии
│   ├── PORTABLE_APPS.md          # Портативные версии
│   ├── СПРАВКА.md                # Справка пользователя
│   └── Публичный интерфейс АРШИНА.md
│
├── 📂 App_Linux/                 # Портативная версия для Linux
│   ├── README.md
│   ├── build_appimage.sh         # Сборка AppImage
│   └── run_arshin.sh             # Запуск
│
├── 📂 App_Windows/               # Портативная версия для Windows
│   ├── README.md
│   ├── build_portable.bat        # Сборка EXE
│   └── run_arshin.bat            # Запуск
│
├── 📂 config/                    # Файлы конфигурации
│   ├── exact_queries.csv         # Точные запросы
│   ├── manufacturers.csv         # Производители
│   └── README.md
│
├── 📂 Konsol/                    # Консольная версия
│   ├── arshin_app_konsol.py      # Консольное приложение
│   ├── api_client.py             # API клиент
│   ├── database.py               # Работа с БД
│   ├── models.py                 # Модели данных
│   └── README.md
│
├── 📂 Konsol_Excel/              # Консольная версия с Excel
│   ├── main.py                   # Основной скрипт
│   ├── excel_handler.py          # Обработка Excel
│   ├── collector.py              # Сбор данных
│   └── README.md
│
├── 📂 тесты/                     # Тестовые файлы
│   └── Тестовый запрос_3.xlsx
│
├── 📂 exports/                   # Экспортированные файлы
├── 📂 logs/                      # Логи приложения
├── 📂 venv/                      # Виртуальное окружение
├── 📂 Картинки/                  # Скриншоты интерфейса
│
├── 🗄 arshin_data.db             # База данных SQLite
├── 🔧 restart.sh                 # Скрипт перезапуска (Linux)
└── 🔧 restart.bat                # Скрипт перезапуска (Windows)
```

---

## ✨ ВОЗМОЖНОСТИ

### GUI версия (`arshin_app.py`)

| Вкладка | Описание |
|---------|----------|
| 🔍 Поиск | Одиночный поиск по реестру |
| 📦 Пакетная | Пакетный поиск из Excel файла |
| 📋 Результаты | Просмотр, фильтрация, экспорт БД |
| 🔧 Фильтры | Управление конфигурацией |
| 📊 Статистика | Статистика по базе данных |

**Функции:**
- ✅ Автоматическое определение колонок Excel
- ✅ Фильтрация (только электросчетчики)
- ✅ Экспорт в CSV/Excel с русскими заголовками
- ✅ Сохранение служебной информации (Id_ПУ, Номер договора, Код ЭДО и т.д.)
- ✅ 20 колонок в таблице результатов

### Консольная версия (`arshin_app_konsol.py`)

**Параметры:**
```bash
python arshin_app_konsol.py -f файл.xlsx -y 2020,2021,2022 -o результат.csv
python arshin_app_konsol.py --stats      # Статистика БД
python arshin_app_konsol.py --clear      # Очистить БД
```

**Преимущества:**
- ✅ Работа через SSH/терминал
- ✅ Не требует GUI/X11
- ✅ Поддержка tmux/screen для фоновой работы
- ✅ Автоматизация через cron

---

## 📚 ДОКУМЕНТАЦИЯ

### Основная документация

| Файл | Описание |
|------|----------|
| `HELP.md` | Справка пользователя (GUI версия) |
| `ПРЕЗЕНТАЦИЯ.md` | Презентация проекта |
| `Doc/README_KONSOL.md` | Руководство по консольной версии |
| `Doc/PORTABLE_APPS.md` | Портативные версии (AppImage, EXE) |
| `Doc/Публичный интерфейс АРШИНА.md` | Документация API ФГИС |

---

## 📦 ВЕРСИИ ПРИЛОЖЕНИЯ

| Версия | Формат | Файл | Платформа |
|--------|--------|------|-----------|
| **GUI** | Python | `arshin_app.py` | Linux/Windows |
| **Console** | Python | `Konsol/arshin_app_konsol.py` | Linux/Windows |
| **Console+Excel** | Python | `Konsol_Excel/main.py` | Linux/Windows |
| **Portable** | AppImage | `App_Linux/` | Linux |
| **Portable** | EXE | `App_Windows/` | Windows |

---

## 🔧 КОНФИГУРАЦИЯ

### Файл `config/exact_queries.csv`

Точные запросы для приоритетного поиска:

```csv
query,category,priority
Счетчик электрической энергии Меркурий 201,electric,high
Счетчик электрической энергии Нева 103,electric,high
```

### Файл `config/manufacturers.csv`

Словарь производителей:

```csv
keyword,manufacturer,category,priority
меркурий,Меркурий,electric,high
нева,Нева,electric,high
энергомера,Энергомера,electric,high
```

---

## 📊 ЭКСПОРТ ДАННЫХ

### Формат экспорта (CSV/Excel)

**21 поле:**
1. ID
2. № реестра
3. Наименование
4. Обозначение типа
5. Модификация
6. Зав. номер
7. Дата поверки
8. Действ. до
9. Пригоден
10. Поверитель
11. № документа
12. Производитель
13. **Id_ПУ** (из Excel)
14. **Номер договора** (из Excel)
15. **Код ЭДО** (из Excel)
16. **Балансовая принадлежность** (из Excel)
17. **Эксплуатационная ответственность** (из Excel)
18. **МПИ** (из Excel)
19. Поисковый запрос
20. № строки
21. Время сохранения

**Все заголовки на русском языке!**

---

## 🧰 ТЕСТИРОВАНИЕ

```bash
# Запуск тестового скрипта
cd тесты
python test_api_and_export.py

# Результаты сохраняются в exports/
```

---

## ⚠️ ВАЖНЫЕ ЗАМЕЧАНИЯ

1. **API лимиты:** API ФГИС АРШИН имеет ограничения на количество запросов
2. **Фильтрация:** Автоматически фильтруются только электросчетчики (~15% от всех записей)
3. **URL записей:** Генерируется из `vri_id`: `https://fgis.gost.ru/fundmetrology/cm/erts/?id={vri_id}`
4. **Миграция БД:** Не требуется, база пересоздаётся автоматически

---

## 📞 ПОДДЕРЖКА

При возникновении проблем:

1. Проверьте логи в папке `logs/`
2. Запустите тестовый скрипт `тесты/test_api_and_export.py`
3. Проверьте доступность API: `curl https://fgis.gost.ru/fundmetrology/eapi/vri`

---

## 📖 ПОМОЩЬ

**Полная документация:** [HELP.md](HELP.md)

**Содержание:**
- [О программе](HELP.md#о-программе)
- [Возможности](HELP.md#возможности)
- [Вкладка "🔍 Поиск"](HELP.md#вкладка--поиск)
- [Вкладка "📦 Пакетная"](HELP.md#вкладка--пакетная)
- [Вкладка "📋 Результаты"](HELP.md#вкладка--результаты)
- [Вкладка "🔧 Фильтры"](HELP.md#вкладка--фильтры)
- [Вкладка "📊 Статистика"](HELP.md#вкладка--статистика)
- [Экспорт данных](HELP.md#экспорт-данных)
- [Частые вопросы](HELP.md#частые-вопросы)

**Быстрые ссылки:**
- [Инструкция по одиночному поиску](HELP.md#поиск)
- [Инструкция по пакетному поиску](HELP.md#поиск-1)
- [Работа с результатами](HELP.md#работа-с-результатами)
- [Экспорт в CSV/Excel](HELP.md#форматы)

---

## 📄 ЛИЦЕНЗИЯ

Приложение для работы с публичным API ФГИС АРШИН.

---

**Версия:** 2.13
**Дата обновления:** 2026-03-03
**Разработчик:** АРШИН Проект
